...
class VideoSource:
    ...
    def set_camera(self, index):
        with self.lock:
            ...
            self._open_camera()
            self.meta = {"name": f"camera {index}", "paused": False}
    ...
    def set_video(self, path, name="video"):
        with self.lock:
            self._close_camera()
...
class Diagnostics:
    ...
    def fsm_cmd(self, action):
        if action == "start":
            if self.fsm and self.fsm.snapshot()["running"]:
                return
            self.fsm = MissionFSM(
                dry_run=True, time_scale=15.0,
                frame_provider=lambda _which: self.src.frame())
            self.fsm.start()
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if p == "/api/mode":
            ...
            if t == "camera":
                DIAG.src.set_camera(int(d.get("index", 0)))
            elif t == "image":
                ...
                tmp.write(raw)
                tmp.close()
                DIAG.src.set_video(tmp.name, name=name or "video")
            elif t == "pause":
                DIAG.src.set_paused(bool(d.get("paused", True)))
            self._json({"ok": True, "info": DIAG.src.info()})
